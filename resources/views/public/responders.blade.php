...
@section('content')
    <form action="{{ route('public.responders.index') }}" id="filter-form" method="GET" >
        <div class="my-2 row">
    ...
    @forelse ($responders as $responder)
        <a @class(['card', 'mt-2']) href="{{ route('public.responders.show', $responder->id) }}">
            <div class="card-body">
                <div class="card-title">{{ $responder->name }}</div>
                <p>{{ str($responder->description)->limit(70) }}</p>
                <p>Location: <strong>{{ $responder->location->fullAddress }}</strong></p>
            </div>
        </a>
    @empty
        <h1 class="text-center">Empty Responders</h1>
    @endforelse
    <div class="my-2">
